# US-004-01 : Enregistrer une dépense simple (2 postings, 1 devise)

## Informations générales

- **Epic parent** : EPIC-004 - Gestion des Transactions Simples
- **Priorité** : Critique
- **Complexité** : Haute
- **Sprint** : Sprint 3

## Prérequis

- **US-003-01** : Créer un compte bancaire (Assets)
- **US-003-02** : Créer un compte de dépenses (Expenses)
- Au moins 1 compte Assets et 1 compte Expenses configurés

## Profil utilisateur

**Utilisateur** qui souhaite enregistrer une dépense courante (courses, restaurant, transport, etc.)

## Objectif business

Permettre à l'utilisateur d'enregistrer ses dépenses quotidiennes pour suivre son budget et analyser ses finances.

## Objectifs concrets

1. Créer une transaction avec 2 postings (partie double)
2. Débiter un compte Expenses (montant positif)
3. Créditer un compte Assets (montant négatif)
4. Valider l'équilibre de la transaction (somme = 0)
5. Valider la cohérence des devises
6. Persister la transaction dans le fichier TOML
7. Afficher la transaction dans la liste
8. Mettre à jour les soldes des comptes

## Scénario nominal

### Étapes

1. L'utilisateur accède à **Transactions**
2. L'utilisateur clique sur **[+ Nouvelle transaction]**
3. Un formulaire s'affiche avec les champs :
   - Date : `2025-01-15`
   - Description : `Courses au supermarché Migros`
   - Bénéficiaire (payee) : `Migros` (optionnel)
   - Tags : `groceries, food` (optionnel)
   - **Postings** :
     - Posting 1 : `Expenses:Food:Groceries` | `120.50 CHF`
     - Posting 2 : `Assets:Bank:CHF:PostFinance` | `-120.50 CHF`
4. L'utilisateur saisit les informations
5. L'application affiche l'indicateur d'équilibre : "✓ Équilibrée (0.00 CHF)"
6. L'utilisateur clique sur **[Enregistrer]**
7. L'application valide les données (V-TXN-*, V-POST-*, V-BAL-001)
8. L'application génère un ID unique (ex: `txn_001`)
9. L'application ajoute la transaction en mémoire
10. L'application recalcule les soldes des comptes concernés
11. L'application sauvegarde le fichier TOML
12. L'application affiche : "✓ Transaction enregistrée avec succès"
13. La transaction apparaît dans la liste

## Données de test

### Données entrantes (formulaire)

```javascript
{
  date: "2025-01-15",
  description: "Courses au supermarché Migros",
  payee: "Migros",
  tags: ["groceries", "food"],
  postings: [
    {
      accountId: "acc_002",  // Expenses:Food:Groceries
      amount: 120.50,
      currency: "CHF"
    },
    {
      accountId: "acc_001",  // Assets:Bank:CHF:PostFinance
      amount: -120.50,
      currency: "CHF"
    }
  ]
}
```

### Données sortantes (fichier TOML)

```toml
[[transaction]]
id = "txn_001"
date = "2025-01-15"
description = "Courses au supermarché Migros"
payee = "Migros"
tags = ["groceries", "food"]

  [[transaction.posting]]
  accountId = "acc_002"
  amount = 120.5
  currency = "CHF"

  [[transaction.posting]]
  accountId = "acc_001"
  amount = -120.5
  currency = "CHF"
```

### Impact sur les soldes

**Avant la transaction :**
- `Assets:Bank:CHF:PostFinance` (acc_001) : 5'000.00 CHF
- `Expenses:Food:Groceries` (acc_002) : 0.00 CHF

**Après la transaction :**
- `Assets:Bank:CHF:PostFinance` (acc_001) : **4'879.50 CHF** (5000 - 120.50)
- `Expenses:Food:Groceries` (acc_002) : **120.50 CHF** (0 + 120.50)

### Affichage dans la liste des transactions

```
📋 Transactions

Date       | Description                        | Montant     | Comptes
-----------|------------------------------------| ------------|------------------
15/01/2025 | Courses au supermarché Migros     | -120.50 CHF | 🛒 Food:Groceries
           | 🏷️ groceries, food                |             | 💳 Bank:CHF

[📄 Détails] [✏️ Modifier] [🗑️ Supprimer]
```

## Critères d'acceptation

- [ ] Le formulaire de transaction est accessible
- [ ] Une transaction contient au minimum 2 postings
- [ ] La somme des montants doit être 0 (tolérance ±0.01)
- [ ] Les devises de tous les postings doivent correspondre aux devises des comptes
- [ ] La date doit être au format YYYY-MM-DD
- [ ] La date ne doit pas être dans le futur (avertissement)
- [ ] Un ID unique est généré automatiquement (format `txn_XXX`)
- [ ] La transaction est sauvegardée dans le fichier TOML
- [ ] Les soldes des comptes sont mis à jour immédiatement
- [ ] Un message de confirmation est affiché
- [ ] La transaction apparaît dans la liste triée par date

## Validation

### Règles appliquées

- **V-TXN-001** : ID au format `txn_XXX`
- **V-TXN-002** : ID unique
- **V-TXN-003** : Date au format YYYY-MM-DD
- **V-TXN-004** : Description non vide
- **V-TXN-005** : Au moins 2 postings
- **V-TXN-006** : Date pas dans le futur (avertissement)
- **V-POST-001** : accountId existe
- **V-POST-002** : amount ≠ 0
- **V-POST-003** : currency correspond au compte
- **V-POST-004** : date >= date d'ouverture du compte
- **V-POST-005** : date <= date de fermeture du compte (si fermé)
- **V-POST-007** : Précision décimale conforme à la devise
- **V-BAL-001** : Somme des montants = 0 (tolérance ±0.01)

### Messages d'erreur

#### Transaction non équilibrée

```
❌ Transaction non équilibrée

Somme actuelle : -20.50 CHF

Une transaction doit être équilibrée (somme = 0).

Postings actuels :
  Expenses:Food:Groceries        : +100.00 CHF
  Assets:Bank:CHF:PostFinance    : -120.50 CHF
  ───────────────────────────────────────────────
  Total                          : -20.50 CHF ❌

Suggestions :
  - Modifiez le montant du premier posting à 120.50
  - Ou ajoutez un posting supplémentaire de +20.50 CHF
```

#### Description vide

```
❌ Description obligatoire

La description de la transaction ne peut pas être vide.

Exemple : "Courses au supermarché Migros"
```

#### Montant zéro dans un posting

```
❌ Montant invalide

Le montant d'un posting ne peut pas être 0.

Posting concerné : Expenses:Food:Groceries

Action : Entrez un montant positif (débit) ou négatif (crédit).
```

#### Devise ne correspond pas au compte

```
❌ Incohérence de devise

Le compte "Assets:Bank:CHF:PostFinance" utilise la devise CHF.
Vous tentez d'enregistrer un montant en EUR.

Actions possibles :
  - Changez le compte pour un compte en EUR
  - Ou changez la devise du posting à CHF
  - Ou utilisez une transaction multi-devises (EPIC-005)
```

#### Date dans le futur

```
⚠️ Date dans le futur

La date 25/12/2025 est dans le futur.

Voulez-vous vraiment enregistrer cette transaction ?

[Oui, enregistrer] [Non, modifier la date]
```

## Interface de saisie guidée

### Formulaire avec assistance

```
📝 Nouvelle transaction

Date *
┌─────────────────────────────────────┐
│ 2025-01-15                      [📅]│
└─────────────────────────────────────┘

Description *
┌─────────────────────────────────────┐
│ Courses au supermarché Migros       │
└─────────────────────────────────────┘

Bénéficiaire
┌─────────────────────────────────────┐
│ Migros                               │ [Suggestions : Migros, Coop, ...]
└─────────────────────────────────────┘

Tags (séparés par des virgules)
┌─────────────────────────────────────┐
│ groceries, food                      │ [Suggestions : groceries, food, shopping]
└─────────────────────────────────────┘

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

💰 Postings (minimum 2)

Posting 1
  Compte       : Expenses:Food:Groceries        [🔍]
  Montant      : 120.50
  Devise       : CHF                            [✓]

Posting 2
  Compte       : Assets:Bank:CHF:PostFinance    [🔍]
  Montant      : -120.50
  Devise       : CHF                            [✓]

[+ Ajouter un posting]

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Équilibre : ✓ 0.00 CHF (équilibrée)

[Annuler]  [Enregistrer]
```

### Indicateur d'équilibre en temps réel

L'indicateur d'équilibre se met à jour automatiquement à chaque modification :

```
✓ 0.00 CHF (équilibrée)          → Transaction valide
⚠️ +25.50 CHF (non équilibrée)   → Transaction invalide
⚠️ -12.00 CHF (non équilibrée)   → Transaction invalide
```

### Suggestions de comptes

Lorsque l'utilisateur tape dans le champ "Compte", afficher des suggestions :

```
[🔍] Assets:Bank:CHF:Post...

Suggestions :
  💳 Assets:Bank:CHF:PostFinance      (Solde : 5'000.00 CHF)
  💳 Assets:Bank:CHF:UBS              (Solde : 2'500.00 CHF)
  💰 Assets:Cash:CHF                  (Solde : 200.00 CHF)
```

## Raccourcis et modèles (templates)

### Modèles de transactions courantes

Pour accélérer la saisie, proposer des modèles :

```
📋 Modèles de transactions

[🛒] Courses alimentaires
  → Expenses:Food:Groceries
  → Assets:Bank:CHF:PostFinance

[🚗] Essence / Transport
  → Expenses:Transport:Fuel
  → Assets:Bank:CHF:PostFinance

[☕] Restaurant / Café
  → Expenses:Food:Restaurants
  → Assets:Bank:CHF:PostFinance

[+ Créer un nouveau modèle]
```

### Duplication de transaction

Permettre de dupliquer une transaction existante :

```
[📋 Dupliquer]

Tous les champs sont pré-remplis.
Modifiez ce qui est nécessaire (date, montant, etc.).
```

## Calcul automatique du dernier posting

Pour faciliter la saisie, calculer automatiquement le montant du dernier posting :

```
Posting 1 : Expenses:Food:Groceries      +120.50 CHF
Posting 2 : Assets:Bank:CHF:PostFinance  [Auto : -120.50 CHF]

[✓] Calculer automatiquement le dernier posting
```

## Notes techniques

### Génération de l'ID

```typescript
function generateTransactionId(existingTransactions: Transaction[]): string {
  const maxId = Math.max(
    0,
    ...existingTransactions.map(t => parseInt(t.id.replace('txn_', '')))
  );
  const nextId = maxId + 1;
  return `txn_${nextId.toString().padStart(3, '0')}`;
}
```

### Validation de l'équilibre

```typescript
function isBalanced(postings: Posting[]): boolean {
  const tolerance = 0.01;
  const sum = postings.reduce((acc, p) => acc + p.amount, 0);
  return Math.abs(sum) <= tolerance;
}
```

### Mise à jour des soldes

```typescript
function updateAccountBalances(transaction: Transaction) {
  for (const posting of transaction.postings) {
    const account = accounts.find(a => a.id === posting.accountId);
    if (account) {
      account.balance += posting.amount;
    }
  }
}
```

**Note** : Les soldes ne sont pas stockés dans le fichier TOML, ils sont recalculés à chaque chargement.

## Dépendances

- US-003-01 : Créer un compte bancaire (Assets)
- US-003-02 : Créer un compte de dépenses (Expenses)
- US-001-03 : Sauvegarder les données en fichier TOML
- US-001-05 : Valider la structure du fichier TOML
